...
> [!NOTE]
> **なぜ Feature と Entity を分けるのか？ (FSDの哲学)**
> 一般的な機能別分割（Vertical Slice Architecture）では、一つの機能（例：ノート機能）のフォルダにUIもドメインも状態管理も全て詰め込みますが、FSDでは**「名詞（モノ）」と「動詞（ユーザーの行動）」を厳格にレイヤーとして分離**します。
> 
> - **`entities` レイヤー（名詞）**: 「インベーダーとは何か」「単語とは何か」という純粋なビジネスルール（ドメイン）を置く場所です。ゲームが増えるごとにこの配下にドメインが大量発生しますが、それはFSDにおいて正しい状態です。
> - **`features` レイヤー（動詞）**: 「ゲームをプレイする」「単語をタイピングする」など、エンティティを組み合わせてユーザーの目的（ユースケース）を達成するための操作（UIやStore）を置く場所です。
> 
> これにより、複数のゲームで共通のエンティティ（例：単語判定ロジック）を安全に使い回すことが可能になります。
...
## 9. データ管理戦略 (JSON外部化と動的ロード)

単語やステージ設定などの**「コンテンツ（データ）」は、TypeScriptのソースコード内に直書き（ハードコード）してはなりません。**
コンテンツとソースコードが混在すると、バンドルサイズの肥大化、コンパイルの遅延を招き、将来的な外部CMS・API化への移行コストが増大します。

本プロジェクトでは以下のルールを徹底します。
1. **データの分離**: データはすべて `public/data/` などの公開領域に静的ファイル（JSON等）として配置する。
2. **非同期ロード**: UI（あるいはStore）の初期化時に `fetch` 等を用いて非同期でデータを取得し、Repository等に注入する。
3. **ローディング状態の管理**: UI側でローディング状態（`isReady` 等）を監視し、ロード完了までは操作不能・描画待機とすることで安全性を担保する。
